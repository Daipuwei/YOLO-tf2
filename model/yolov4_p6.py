...
def yolov4_p6_body(feats, num_anchors, num_classes):
    """
    这是YOLOv4-p6主干的定义函数
    ...
    return [P6_output,P5_output, P4_output, P3_output]

def build_yolov4_p6_train(cfg):
    """
    这是搭建训练阶段YOLO v4的函数
    :param cfg: 参数配置类
    :return:
    ...
    if cfg.MODEL.BACKBONE.NAME == 'CPSDarkNet':               # CSPDarknet
        from model.backbone.cspdarknet import yolov4_p6_cspdarknet_backbone
        feats = yolov4_p6_cspdarknet_backbone(image_input,base=cfg.MODEL.BACKBONE.BASE,
    ...
    # 搭建YOLOv4-p6主干
    print('Create YOLOv4-p6 model with {} anchors and {} classes.'.format(num_anchors,num_classes))
    yolov4_p6_outputs = yolov4_p6_body(feats, num_anchors//4, num_classes)
    yolov4_p6_body_model = Model(image_input,yolov4_p6_outputs)
    if cfg.MODEL.MODEL_PATH is not None:
        yolov4_p6_body_model.load_weights(os.path.abspath(cfg.MODEL.MODEL_PATH), by_name=True, skip_mismatch=True)
        print('Load weights from: {}.'.format(os.path.abspath(cfg.MODEL.MODEL_PATH)))
    yolov4_p6_body_model.summary()

    # 搭建训练阶段YOLOv4
    y_true = [Input(shape=(None, None, num_anchors // 4, num_classes + 5)) for l in range(4)]
    loss_input = [*yolov4_p6_body_model.output, *y_true]
    ...
    yolov4_p6_train_model = Model([image_input, *y_true], model_loss)
    return yolov4_p6_body_model,yolov4_p6_train_model

def build_yolov4_p6_eval(cfg):
    """
    这是搭建评估阶段YOLO v4的函数
    :param cfg: 参数配置类
    :return:
    ...
    inputs = [*yolov4_p6_body_model.output, input_image_shape]
    outputs = Lambda(yolo_eval, output_shape=(1,), name='yolov4_p6_preds',
                     arguments={'anchors': anchors,
                                'num_classes': num_classes,
    ...
    yolov4_p6_eval_model = Model([image_input, input_image_shape], outputs)
    return yolov4_p6_eval_model